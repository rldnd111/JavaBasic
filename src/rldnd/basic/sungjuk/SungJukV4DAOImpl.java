package rldnd.basic.sungjuk;

import rldnd.basic.jdbc.JDBCUtil;
...
public class SungJukV4DAOImpl implements SungJukV4DAO {
    ...
    private String selectSQL = " select sjno,name,kor,eng,mat,mid(regdate,1,10) regdate " +
    ...
    private String daelteSQL = " delete from sungjuk where sjno = ? ";
    ...
    // 조회해서 ArrayList에 담아서 Service로 넘김
    @Override
    ...
    public int deleteSungJuk(int sjno) {
        ...
        try(
                Connection conn = jdbc.openConn();
                PreparedStatement pstmt = conn.prepareStatement(daelteSQL)
                ) {
            pstmt.setInt(1,sjno);
            cnt = pstmt.executeUpdate();
        } catch (Exception ex) {
