package rldnd.basic.day15;

import java.io.*;

public class ByteStream {
    public static void main(String[] args) {
        // 바이트단위 스트림 - 스트림을 바이트로만 처리함
        // 파일을 바이트 단위로 입출력하려면
        // InputStream 또는 OutputStream 클래스를 사용함

        // 바이트기반으로 스트림을 읽거나 씀 : 바이트 기반(1byte)
        // 주로 바이너리 기반 데이터를(이미지,동영상등) 처리할때 사용
        // 기계가 읽고 쓸 수있는 데이터를 대상으로 함
        ...
        try {
            ...
            bis.read(readData);
            // bis를 이용해서 512바이트의 데이터를 일겅서
            // readDate 변수에 저장함

            String result = "";
            for(int i = 0 ; i < readData.length; ++i) {
                result += (char)readData[i];
            } // readDate에 저장된 바이트를 하나씩 꺼내 문자로 변환
            System.out.println(result);
            // 한글처리 기능이 없으므로 한글이 깨짐
            // 남아있는 채워지지 않은 자리에는 빈값으로 저장됨
